...
const $chatbotBtnToggle = $document.getElementById("chatbot_toggle")
...
let started_conversation = false;
const command_start_dilago = "\\comenzar_dialogo"
// webchat
...
socketclient.on('connect', function(){
    ...
    socketclient.on('message', (response) => {
        const message_text = response['message']
        console.log("message_text: ", message_text)
        setResponseBot(message_text, "text",  botLoadingDelay + botReplyDelay)
      });
})
...
$chatbotBtnToggle.addEventListener("click", () => {
    toggle($chatbot, 'chatbot--closed');
    if (started_conversation != true){
        send_websocket(command_start_dilago);
...
const send_websocket = (text = '')=> {
    socketclient.emit("message", text)
}
